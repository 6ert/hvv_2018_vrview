...
d){for(var e=$jscomp.makeIterator(a),g=e.next();!g.done;g=e.next())c(g.value).callWhenSettled_(b,d)})};d.all=function(a){var b=$jscomp.makeIterator(a),e=b.next();return e.done?c([]):new d(function(a,d){function g(b){return function(c){f[b]=c;h--;0==h&&a(f)}}var f=[],h=0;do f.push(void 0),h++,c(e.value).callWhenSettled_(g(f.length-1),d),e=b.next();while(!e.done)})};return d},"es6","es3");var shakaDemo=shakaDemo||{};shakaDemo.onlineOptGroups_=[];
shakaDemo.setupAssets_=function(){var a=document.getElementById("assetList"),b={},c=null;shakaAssets.testAssets.forEach(function(d){if(!d.disabled){var e=b[d.source];e||(e=document.createElement("optgroup"),e.label=d.source,e.disabled=!navigator.onLine,b[d.source]=e,a.appendChild(e),shakaDemo.onlineOptGroups_.push(e));var f=document.createElement("option");f.textContent=d.name;f.asset=d;e.appendChild(f);d.drm.length&&!d.drm.some(function(a){return shakaDemo.support_.drm[a]})&&(f.disabled=!0);var k=
[];0<=d.features.indexOf(shakaAssets.Feature.WEBM)&&k.push("video/webm");0<=d.features.indexOf(shakaAssets.Feature.MP4)&&k.push("video/mp4");0<=d.features.indexOf(shakaAssets.Feature.MP2TS)&&k.push("video/mp2t");k.some(function(a){return shakaDemo.support_.media[a]})||(f.disabled=!0);f.disabled||e.disabled||(c=c||f,d.focus&&(c=f))}});c&&(c.selected=!0);var e=shakaDemo.setupOfflineAssets_(),d=document.createElement("option");d.textContent="(custom asset)";a.appendChild(d);a.addEventListener("change",
function(){var b=a.options[a.selectedIndex].asset;document.getElementById("customAsset").style.display=b?"none":"block";shakaDemo.hashShouldChange_()});document.getElementById("loadButton").addEventListener("click",shakaDemo.load);document.getElementById("unloadButton").addEventListener("click",shakaDemo.unload);document.getElementById("licenseServerInput").addEventListener("input",shakaDemo.onAssetInput_);document.getElementById("manifestInput").addEventListener("input",shakaDemo.onAssetInput_);
document.getElementById("certificateInput").addEventListener("input",shakaDemo.onAssetInput_);return e};shakaDemo.onAssetInput_=function(a){shakaDemo.hashShouldChange_()};shakaDemo.requestCertificate_=function(a){return shakaDemo.player_.getNetworkingEngine().request(shaka.net.NetworkingEngine.RequestType.APP,{uris:[a]}).promise.then(function(a){return a.data})};
...
document.getElementById("preferredTextLanguage").value;d=Number(document.getElementById("preferredAudioChannelCount").value);isNaN(d)||(f.preferredAudioChannelCount=d);f.abr.enabled=document.getElementById("enableAdaptation").checked;d=document.getElementById("smallGapLimit").value;!isNaN(Number(d))&&0<d.length&&(f.streaming.smallGapLimit=Number(d));f.streaming.jumpLargeGaps=document.getElementById("jumpLargeGaps").checked;f.streaming.alwaysStreamText=document.getElementById("showNative").checked;
b.configure(f);window.debugConfig&&b.configure(window.debugConfig);return a};shakaDemo.computeDisabledAssets=function(){shakaDemo.onlineOptGroups_.forEach(function(a){a.disabled=!navigator.onLine})};
...
shakaDemo.onDrmSettingsChange_);document.getElementById("drmSettingsAudioRobustness").addEventListener("input",shakaDemo.onDrmSettingsChange_);var a=document.getElementById("robustnessSuggestions");shakaDemo.support_.drm["com.widevine.alpha"]&&["SW_SECURE_CRYPTO","SW_SECURE_DECODE","HW_SECURE_CRYPTO","HW_SECURE_DECODE","HW_SECURE_ALL"].forEach(function(b){var c=document.createElement("option");c.value=b;c.textContent="Widevine";a.appendChild(c)})};shakaDemo.onLoadOnRefreshChange_=function(){shakaDemo.hashShouldChange_()};
shakaDemo.onDrmSettingsChange_=function(a){shakaDemo.hashShouldChange_()};shakaDemo.onLogLevelChange_=function(a){if(shaka.log){switch(a.target[a.target.selectedIndex].value){case "info":shaka.log.setLevel(shaka.log.Level.INFO);break;case "debug":shaka.log.setLevel(shaka.log.Level.DEBUG);break;case "vv":shaka.log.setLevel(shaka.log.Level.V2);break;case "v":shaka.log.setLevel(shaka.log.Level.V1)}shakaDemo.hashShouldChange_()}};
shakaDemo.onJumpLargeGapsChange_=function(a){shakaDemo.player_.configure({streaming:{jumpLargeGaps:a.target.checked}});shakaDemo.hashShouldChange_()};shakaDemo.onGapInput_=function(a){var b=Number(a.target.value);a=isNaN(b)||0==a.target.value.length;shakaDemo.player_.configure({streaming:{smallGapLimit:a?void 0:b}});shakaDemo.hashShouldChange_()};
...
d!=c?(a.push("audiolang="+c),a.push("textlang="+d)):a.push("lang="+c);c=document.getElementById("preferredAudioChannelCount").value;"2"!=c&&a.push("channels="+c);document.getElementById("logToScreen").checked&&a.push("logtoscreen");document.getElementById("enableAdaptation").checked||a.push("noadaptation");document.getElementById("showTrickPlay").checked&&a.push("trickplay");document.getElementById("showNative").checked&&a.push("nativecontrols");shaka.log&&(c=document.getElementById("logLevelList"),
c=c[c.selectedIndex].value,"info"!=c&&a.push(c));document.getElementById("enableLoadOnRefresh").checked&&a.push("play");"noinput"in b&&a.push("noinput");void 0!=shakaDemo.startTime_&&a.push("startTime="+shakaDemo.startTime_);(c=document.getElementById("drmSettingsVideoRobustness").value)&&a.push("videoRobustness="+c);(c=document.getElementById("drmSettingsAudioRobustness").value)&&a.push("audioRobustness="+c);var h="uncompiled",k="#"+a.join(";");"build"in b?(a.push("build="+b.build),h=b.build):"compiled"in
b&&(a.push("build=compiled"),h="compiled");["compiled","debug_compiled","uncompiled"].forEach(function(a){var b=document.getElementById(a+"_link");h==a?(b.classList.add("disabled_link"),b.removeAttribute("href"),b.title="currently selected"):(b.classList.remove("disabled_link"),b.onclick=function(){location.hash=k+";build="+a;location.reload();return!1})});try{eval("()=>{}")}catch(l){b=document.getElementById("uncompiled_link"),b.classList.add("disabled_link"),b.removeAttribute("href"),b.title="requires ES6"}a=
"#"+a.join(";");a!=location.hash&&(shakaDemo.suppressHashChangeEvent_=!0,location.hash=a);location.search&&(location.search="")}};shakaDemo.onErrorEvent_=function(a){shakaDemo.onError_(a.detail)};
...
shakaDemo.updateButtons_=function(a){var b=document.getElementById("assetList"),c=shakaDemo.offlineOperationInProgress_;document.getElementById("progressDiv").style.display=a&&!c?"none":"block";a=b.options[b.selectedIndex];b=a.storedContent;var e=!a.asset||!a.asset.drm||!a.asset.drm.length||a.asset.drm.some(function(a){return shakaDemo.support_.drm[a]&&shakaDemo.support_.drm[a].persistentState});document.getElementById("offlineNameDiv").style.display=a.asset?"none":"block";var d=document.getElementById("storeDeleteButton");
d.disabled=c||!e||a.isStored;d.textContent=b?"Delete":"Store";document.getElementById("storeDeleteHelpText").textContent=c?"Operation is in progress...":e?d.disabled?'The asset is stored offline. Checkout the "Offline" section in the "Asset" list':"":"This browser does not support persistent licenses."};
